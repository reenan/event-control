...
export default class EventList extends Component {
	...
	render() {
		const { list, setActiveItem, edit, canMark, afterChangeFavorite } = this.props;
		...
		return (
			<div className="event-list">
				<ul>
					{
						list.map((item, index) => {
							return <EventListItem afterChangeFavorite={afterChangeFavorite} canMark={canMark} setActiveItem={setActiveItem} edit={edit} key={index} item={item} />
						})
					}/
...
class EventListItem extends Component {
    ...
    toggleFavorito = () => {
        $.ajax({
            ...
            success: (response) => {
                this.props.afterChangeFavorite();
            }
        })
    }
		...
		return (
			<li onClick={this.handleClick} className="event-list-item">
				...
				<div className="data">
					...
					<Content className={`descricao ${descricaoSize}`}>
						{ item.descricao }
					</Content>
